resource "aws_api_gateway_integration" "http_resource_integration" {
  ...
  request_parameters = "${merge(zipmap(split(",", var.auth_enabled ? "integration.request.header.Authorization" : "integration.request.header.Host"), split(",", var.auth_enabled ? "method.request.header.Authorization" : "method.request.header.Host")), var.custom_request_parameters)}"
}
