resource "aws_api_gateway_integration" "http_resource_integration" {
  count       = "${length(keys(var.method_lambda_map))}"
  ...
  uri                     = "arn:aws:apigateway:${data.aws_region.current.name}:lambda:path/2015-03-31/functions/${element(values(var.method_lambda_map), count.index)}/invocations"
